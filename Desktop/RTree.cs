...
namespace Slb.AvocetVM.Utilities.RTree
{
    ...
    public class RTree<T>
    {
        ...
        public delegate void itemproc(T x);
        public delegate bool intersect_func(Rectangle a, Rectangle b);

        private static bool default_intersects_func(Rectangle a, Rectangle b)
        {
        ...
        public void Intersects(Rectangle r, itemproc v, intersect_func inter = null)
        {
        ...
        private void intersects(Rectangle r, intproc v, intersect_func inter = null)
        {
            Node<T> rootNode = getNode(rootNodeId);
            intersects(r, v, rootNode, inter);
        }
        ...
        */
        ...
        /// <param name="v"></param>
        /// <param name="n"></param>
        private void intersects(Rectangle r, intproc v, Node<T> n, intersect_func inter = null)
        {
            inter = inter ?? default_intersects_func;
            for (int i = 0; i < n.entryCount; i++)
            {
                if (inter(r, (n.entries[i])))
                {
                    if (n.isLeaf())
                    {
                        v(n.ids[i]);
                    ...
                    else
                    {
                        ...
                        intersects(r, v, childNode, inter);
                    }
                }
            }
        }
